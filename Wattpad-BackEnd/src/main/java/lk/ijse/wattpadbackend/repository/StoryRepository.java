...
public interface StoryRepository extends JpaRepository<Story,Integer> {
    ...
    @Query(value = "SELECT * FROM story WHERE user_id <> :userId AND id <> :storyId ORDER BY RAND() LIMIT 2", nativeQuery = true)
    List<Story> findTwoRandomStoriesNotBelongingToUserAndStory(
