...
window.onload = function () {
    ...
    fetch('http://localhost:8080/api/v1/user/mute', {
        method: 'GET',
        credentials: 'include',
        headers: {
...
//load all muted users
async function loadAllMutedUsers() {

    await fetch('http://localhost:8080/api/v1/user/mute/all', {
        method: 'GET',
        credentials: 'include',
        headers: {
            'Content-Type': 'application/json'
        ...
        .then(data => {
            ...
            if(data.data.length<=0){
                $('.subheading').text('These are no users you have muted');
            }
            else{
                $('.subheading').text('These are the users you have muted');
            }
            ...
            for (let i = 0; i < data.data.length; i++) {
                ...
                let singleUser = `<!--single muted user-->
                                    <li class="muted-user">
                                        <!--user image-->
                                        <img src="${user.profilePicPath}" alt="user avtar" align="top" width="64" height="64">
                                        <!--user name-->
