...
//load stories in selected tag by selected criteria
async function loadAllStoriesInSelectedTag(data) {
    ...
    $('#genre-title').text(tag+' Stories');


    await fetch('http://localhost:8080/api/v1/tag/'+tag, {
        method: 'POST',
        credentials: 'include',
        headers: {
            'Content-Type': 'application/json'
...
loadAllStoriesInSelectedTag(data);
